...
ENT.PrintName = "Balancing Pipe (Large)"
ENT.Author = ""
...
ENT.Model = "models/parkoursource/pipe_beam.mdl"
ENT.NoClimbing = true
ENT.Balance = true
...
function ENT:BalanceLengthExact(length)
	...
	local mins, maxs = Vector(-16, -3, -0), Vector(6, 6, length)
...
function ENT:Initialize()
	...
	local mins, maxs = Vector(-5, -6, -0), Vector(7, 6, height)
	...
	--self:SetPos(self:GetPos() - ang:Forward() * 0)
end
...
function ENT:Think()
	if SERVER then -- When you spawn this entity sometimes on certain spots on the map will send the pipe under the floor, No clue how to resolve that error..
		local ang = self:GetAngles()
...
function ENT:Draw()
	...
	for i = 0, numc do
		...
		if i == numc then
			pos = pos + ang:Up() * (242 * (i - 1))

			if i > 0 then
				pos = pos + ang:Up() * (242 * extra)
			end
		...
		--	self.CLModel:DrawModel()
			...
			pos = pos + ang:Up() * (242 * i) -- Mesh length adjustments
